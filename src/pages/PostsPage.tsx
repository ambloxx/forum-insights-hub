...
function buildPostUrl(subject: string) {
  if (!subject) return '#';
  return `https://help.zoho.com/portal/en/community/topic/${subject.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')}`;
}
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              filtered.map(post => {
                const url = post.topic_url || buildPostUrl(post.subject);
                const lastActivity = post.latest_comment_time || post.created_time;
                return (
                  <TableRow
                    key={post.id}
                    className={`hover:bg-secondary/30 transition-colors cursor-pointer ${post.unreplied ? 'border-l-2 border-l-destructive' : ''}`}
