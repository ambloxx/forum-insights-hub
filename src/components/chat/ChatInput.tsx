...
import { ArrowUp, BookOpen, Globe, Square } from 'lucide-react';
  ...
  return (
    <div className="px-4 pb-5 pt-2">
      <div className="max-w-[680px] mx-auto space-y-3">
        <div className="relative rounded-2xl bg-muted/40 border border-border/50 transition-all duration-200 focus-within:bg-muted/60 focus-within:border-border">
          ...
          {mode !== 'default' && (
            <div className="flex items-center gap-2 px-4 pt-3">
              <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full ${
                ...
                <button onClick={() => setMode('default')} className="ml-1 hover:opacity-70 transition-opacity">×</button>
              </span>
            </div>
          )}
          ...
          <textarea
            ...
            rows={1}
            className="w-full resize-none bg-transparent px-4 pt-3 pb-2 text-[14px] text-foreground placeholder:text-muted-foreground/50 focus:outline-none leading-relaxed"
          ...
          <div className="flex items-center justify-between px-3 pb-2.5">
            ...
            {isStreaming ? (
              <button
                onClick={onStop}
                className="h-8 w-8 rounded-full bg-destructive text-destructive-foreground flex items-center justify-center hover:bg-destructive/90 transition-all duration-150"
              >
          ...
          <div className="flex flex-wrap gap-1.5 justify-center">
            {suggestions.map(q => (
              <button
                key={q}
                onClick={() => onSend(q)}
                className="text-[12px] px-3 py-1.5 rounded-full border border-border/50 text-muted-foreground hover:text-foreground hover:bg-muted/40 hover:border-border transition-all duration-150"
              >
                {q}
              </button>
            ))}
